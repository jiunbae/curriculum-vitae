...
\outerlist{
    ...
    \innerlist{
        \entry{\ifen{Researched and developed state-of-the-art speech-to-text engines for Korean and Japanese, achieving industry-leading performance across multiple metrics including WER and CER. Introduced LLM-based readability evaluation with a novel assessment dataset to quantitatively measure transcription quality that traditional metrics cannot capture.}\ifko{WER/CER 등 여러 지표에서 한국어/일본어 최고 수준의 STT 엔진을 연구 및 개발했으며, 새로운 LLM 기반 가독성 평가 데이터셋을 도입하여 기존 메트릭으로 측정할 수 없는 부분을 정량평가했습니다.}}
